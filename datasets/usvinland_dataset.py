...
class USVInlandDataset(Dataset):
    def __init__(self, datapath, list_filename, training, distill=False):
    ...
    def load_path(self, list_filename):
        ...
        if len(splits[0]) == 2: # ground truth not available
            ...
            if self.distill: disp_images = [x[2].replace('Disp_Map', 'Disp_Map_Distil') for x in splits] # using Knowledge Distillation
            else: disp_images = [x[2] for x in splits]
    ...
    def load_seg(self, filename):
        filename = os.path.normpath(filename)
        seg = Image.open(filename).convert('RGB')
        seg = np.array(seg)
        seg = (seg[:, :, 0] == 0) * 1.
        return seg
    ...
    def load_disp(self, filename):
        ...
        data = np.array(data, dtype=np.float32) / 255. * 50.
        return data
    ...
    def __getitem__(self, index):
        ...
        left_seg_path = os.path.join(self.datapath, self.left_filenames[index]).replace('Low_Res_640_320', 'Segmentation').replace('Left_Img_Rectified', 'Left_Img_Rectified_Seg_gt').replace('.jpg', '.png')
        right_seg_path = os.path.join(self.datapath, self.right_filenames[index]).replace('Low_Res_640_320', 'Segmentation').replace('Right_Img_Rectified', 'Right_Img_Rectified_Seg_gt').replace('.jpg', '.png')
        left_seg = self.load_seg(left_seg_path) if os.path.exists(left_seg_path) else None
        right_seg = self.load_seg(right_seg_path) if os.path.exists(right_seg_path) else None
